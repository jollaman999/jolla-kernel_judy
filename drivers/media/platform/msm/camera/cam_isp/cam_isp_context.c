	...
	uint32_t next_state)
{
	...
	if (ctx_isp->active_req_cnt >= 4 ) { /* LGE_CHANGES 2->4 */
		CAM_ERR_RATE_LIMIT(CAM_ISP,
	...
	struct cam_req_mgr_flush_request *flush_req)
{
	...
	/* LGE_CHANGE_S, CST, synchronize flush req in activated state */
	spin_lock_bh(&ctx->lock);
	rc = __cam_isp_ctx_flush_req(ctx, &ctx->pending_req_list, flush_req);
...
//		spin_unlock_bh(&ctx->lock);
	...
	spin_unlock_bh(&ctx->lock);
	/* LGE_CHANGE_E, CST, synchronize flush req in activated state */
	return rc;
}
