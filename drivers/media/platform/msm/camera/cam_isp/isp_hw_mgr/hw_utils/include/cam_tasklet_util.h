...
/* LGE_CHANGE, CST, added tasklet stop after kill */
/*
 * cam_tasklet_stop_after_kill()
 *
 * @brief:              Make sure kill tasklet before disabling the tasklet so any scheduled
                         can raise soft irq without changing tasklet state, which can lead infinite loop.
 *                      Need to enable again to run.
 *
 * @tasklet:            Tasklet to kill and stop
 *
 * @return:             Void
 */
void cam_tasklet_stop_after_kill(void    *tasklet);
/*
