	...
	int32_t index)
{
	...
	for (i = 0, j = 0, k = 0; i < packet->num_io_configs; i++) {
		if (io_cfg_ptr[i].direction == CAM_BUF_INPUT) {
			if (io_cfg_ptr[i].resource_type == CAM_ICP_IPE_INPUT_IMAGE_FULL_REF 
			|| io_cfg_ptr[i].resource_type == CAM_ICP_IPE_INPUT_IMAGE_DS4_REF
			|| io_cfg_ptr[i].resource_type == CAM_ICP_IPE_INPUT_IMAGE_DS16_REF)
				continue;
		...
		CAM_DBG(CAM_ICP, "dir[%d]: %u, fence: %ui resource_type = %u",
			i, io_cfg_ptr[i].direction, io_cfg_ptr[i].fence, io_cfg_ptr[i].resource_type);
	}
	...
	void *prepare_hw_update_args)
{
	...
	CAM_DBG(CAM_ICP, "E: req id = %lld ctx_id = %u", 
