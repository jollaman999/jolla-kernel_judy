	...
	const char *name)
{
	...
	CAM_DBG(CAM_SYNC, "Sync object Initialised: sync_id:%u row_state:%u %s",
		row->sync_id, row->state, name);
...
int cam_sync_deinit_object(struct sync_table_row *table, uint32_t idx)
{
	...
	if (!table || idx <= 0 || idx >= CAM_SYNC_MAX_OBJS)
		return -EINVAL;
	/* LGE_CHANGE, CST, change log comment for fence error */
	CAM_DBG(CAM_SYNC,"row name:%s sync_id:%i [idx:%u] row_state:%u",
	    row->name, row->sync_id, idx, row->state);
	spin_lock_bh(&sync_dev->row_spinlocks[idx]);
	if (row->state == CAM_SYNC_STATE_INVALID) {
		CAM_ERR(CAM_SYNC,
		"Error: accessing an uninitialized sync obj: idx = %d %s",
			idx, row->name);
	...
	while (!list_empty(&temp_child_list)) {
		...
		if (child_row->state == CAM_SYNC_STATE_INVALID) {
			/* LGE_CHANGE, CST, list deletion within critical section */
			list_del_init(&child_info->list);
