	...
	*/ //for debugging(LG setting)
	...
	/*
		...
		input_report_abs(data->input_dev_ps, ABS_HAT2X,
			data->offset_cal_data.cal_count);
		input_report_abs(data->input_dev_ps, ABS_HAT2Y,
			data->offset_cal_data.cal_status);
		input_report_abs(data->input_dev_ps, ABS_HAT3X,
			data->offset_cal_data.error_status);
	} else {
		input_report_abs(data->input_dev_ps, ABS_HAT2X,
				...
				size_t count)
{
	...
	if (val == 1) {
		...
		input_sync(data->input_dev_ps);
		pr_err("Unclog the input sub-system\n");
	}
	vl53l0_dbgmsg("End\n");
	mutex_unlock(&data->work_mutex);
				...
				size_t count)
{
	...
	mutex_lock(&data->work_mutex);
/*
	vl53l0_dbgmsg("Enter, laser_offset_calibrate flag:%d\n",
		data->offset_cal_data.laser_offset_calibrate);
	vl53l0_dbgmsg("laser offset calibrate ( %ld)\n", val);
*/
...
/* Step 1. Ref,Spad Cal data check */
	pr_err("OffsetCalibration Step 1. Ref,Spad Cal data check\n");
	...
	pr_err("VL53L0 calCount:%d, refSpadCount:0x%x, count_sep1:0x%x, count_sep2:0x%x, count_sep3:0x%x, count_sep4:0x%x\n",
...
/* Step 2. set "XTalkCompMegaCps" as "0" */
	pr_err("OffsetCalibration Step 2. set XTalkCompMegaCps as 0\n");
	XTalkCompMegaCps = 0;
...
/* Step 3. set "offset" as "0" */
	pr_err("OffsetCalibration Step 3. set offset as st_offset\n");
	if (papi_func_tbl->SetOffsetCalibrationDataMicroMeter != NULL) {
		pr_err("Call of SetOffsetCalibrationDataMicroMeter\n");
		Status =
		papi_func_tbl->SetOffsetCalibrationDataMicroMeter(
			vl53l0_dev, 0);
		if (Status != VL53L0_ERROR_NONE) {
...
/* Step 4. offset calibration */
	pr_err("OffsetCalibration Step 4. offset calibration\n");
	...
	pr_err("offset:%d lge_offset:%d OffsetCalibrationDataMicroMeter: %d error status:%d\n",offset, lge_offset, st_offset, Status);
...
/* Step 5. cal_count ++ */
	pr_err("OffsetCalibration Step 5. cal_count ++\n");
	calCount++;
	data->offset_cal_data.cal_count = calCount;

/* Step 6. EEPROM Write(offset,calCount) */
	pr_err("OffsetCalibration Step 6. EEPROM Write(offset,calCount)\n");
	if (is_perform_ref_cal == 1)
	{
		...
		pr_err("VL53L0 ldaf_i2c_e2p_write_seq end(write ref,offset) rc:%d\n",rc);
	...
	else
	{
		...
		ldaf_cal_data[1] = (uint8_t)lge_offset;
		rc	= ldaf_i2c_e2p_write_seq(IT_EEP_REG,ldaf_cal_data,sizeof(ldaf_cal_data));
		pr_err("VL53L0 ldaf_i2c_e2p_write_seq end(offset) rc:%d\n",rc);
	}
...
/* Step 3. Read EEPROM data */
	...
	pr_err("VL53L0 e2p_offset:%d, lge_offset:%d, calCount:%d\n",e2p_offset,lge_offset,calCount);
	...
	pr_err("VL53L0 refSpadCount:0x%x, count_sep1:0x%x, count_sep2:0x%x, count_sep3:0x%x, count_sep4:0x%x\n",
...
/* Step 4. Data check and set ref,spad calibration */
	...
	if((VhvSettings == 255) | (calCount == 0 )) { // changed because of phasecal = 0 issue
		...
		count_sep3 = (refSpadCount >> 8)  & 0x00FF;
		count_sep4 = (refSpadCount)       & 0x00FF;
		pr_err("VL53L0 count:0x%x, count_sep1:0x%x, count_sep2:0x%x, count_sep3:0x%x, count_sep4:0x%x\n",
		refSpadCount, count_sep1, count_sep2, count_sep3, count_sep4);
		pr_err("VL53L0 VhvSettings:%d, PhaseCal:%d, isApertureSpads:%d\n",VhvSettings, PhaseCal, isApertureSpads);
		...
		pr_err("VL53L0 ldaf_i2c_e2p_write_seq end rc:%d\n",rc);
#else
		is_ref_cal_write = 1;
#endif
	}
...
/* Step 6. Write EEPROM data */
	...
	else if (is_ref_cal_write == 1)
	{
		...
		ldaf_cal_data[6] = isApertureSpads;
		rc  = ldaf_i2c_e2p_write_seq(IT_EEP_REG+2,ldaf_cal_data,sizeof(ldaf_cal_data));
		pr_err("VL53L0 ldaf_i2c_e2p_write_seq end(write ref) rc:%d\n",rc);
	}
	else if(is_offset_cal_write == 1)
	{
		...
		ldaf_cal_data[1] = e2p_offset;
		rc  = ldaf_i2c_e2p_write_seq(IT_EEP_REG,ldaf_cal_data,sizeof(ldaf_cal_data));
		pr_err("VL53L0 ldaf_i2c_e2p_write_seq end(write offset) rc:%d\n",rc);
	}
