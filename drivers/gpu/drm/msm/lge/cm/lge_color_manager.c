		...
		struct device_attribute *attr, char *buf)
{
	...
	if (panel->lge.ddic_ops && panel->lge.ddic_ops->image_enhance_get)
		ret = panel->lge.ddic_ops->image_enhance_get(panel);
	else
		pr_info("Not support\n");
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	sscanf(buf, "%d", &input);
	pr_debug("input data %d\n", input);

	if (panel->lge.ddic_ops && panel->lge.ddic_ops->image_enhance_set)
		panel->lge.ddic_ops->image_enhance_set(panel, input);
	else
		pr_info("Not support\n");
...
static DEVICE_ATTR(image_enhance_set, S_IRUGO | S_IWUSR | S_IWGRP,
		image_enhance_get, image_enhance_set);
...
static ssize_t vr_low_persist_read (struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct dsi_panel *panel;
	int len = 0;
	...
	return sprintf(buf, "%d\n", panel->lge.vr_low_persist_enable);
}

static ssize_t vr_low_persist_setup(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	ssize_t ret = strnlen(buf, PAGE_SIZE);
	struct dsi_panel *panel;
	...
	sscanf(buf, "%d", &enable);
	panel->lge.vr_low_persist_enable = enable;

	if (panel->lge.ddic_ops && panel->lge.ddic_ops->vr_enable && panel->lge.vr_low_persist_enable)
		panel->lge.ddic_ops->vr_enable(panel);
	else if (panel->lge.ddic_ops && panel->lge.ddic_ops->vr_disable && !panel->lge.vr_low_persist_enable)
		panel->lge.ddic_ops->vr_disable(panel);

	pr_info("%s vr low_persist mode  = %d\n",__func__,panel->lge.vr_low_persist_enable);
	return ret;
}
...
int lge_color_manager_create_sysfs(struct device *panel_sysfs_dev)
{
	...
	if ((rc = device_create_file(panel_sysfs_dev, &dev_attr_image_enhance_set)) < 0)
		pr_err("add image_enhance set node fail!");
