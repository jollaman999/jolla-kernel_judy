#define pr_fmt(fmt)	"[Display][sw49410-ops:%s:%d] " fmt, __func__, __LINE__
...
const struct drs_res_info sw49410_res[3] = {
	{"qhd", 0, 1440, 3120},
	{"fhd", 1, 1080, 2340},
	{"hd", 3, 720, 1560},
};
...
static void adjust_roi(struct dsi_panel *panel, int *sr, int *er)
{
	...
	for (type = 0; type < num; type++) {
		if (cur_res == sw49410_res[type].height)
			break;
	}
	if (type == num) {
			...
			int cmds_count, int addr, int sr)
{
	...
	cmd = find_cmd(cmds, cmds_count, addr);
	if (cmd) {
		if (strncmp(sw49410_res[type].resolution, "qhd", 3)) {
			payload = (char *)cmd->msg.tx_buf;
...
static int control_bist_cmds_sw49410(struct dsi_panel *panel, bool enable)
{
	int rc = 0;
...
static int release_bist_cmds_sw49410(struct dsi_panel *panel)
{
	int rc = 0;
	...
	if (panel->lge.bist_on > 0) {
		panel->lge.bist_on = 1;
		if (control_bist_cmds_sw49410(panel, false) < 0) {
...
void sharpness_set_send_sw49410(struct dsi_panel *panel)
{
	u8 *payload;
	int mode;
...
static void lge_set_rgb_tune_send_sw49410(struct dsi_panel *panel, enum lge_gamma_correction_mode gc_mode)
{
	int i = 0;
...
static void lge_set_screen_tune_send_sw49410(struct dsi_panel *panel,
...
void mplus_mode_send_sw49410(struct dsi_panel *panel, enum lge_mplus_mode req_mp_mode)
{
	u8 data;
	u8 *payload;
	...
	lge_set_rgb_tune_send_sw49410(panel, gc_mode);
	lge_set_screen_tune_send_sw49410(panel, lge_screen_tune_status);

	mutex_unlock(&panel->panel_lock);

	sharpness_set_send_sw49410(panel);
...
void mplus_mode_set_sub_sw49410(struct dsi_panel *panel)
{
	if (panel->lge.mplus_hd != LGE_MP_OFF)
		mplus_mode_send_sw49410(panel, panel->lge.mplus_hd);
	else if (panel->lge.hdr_mode)
		mplus_mode_send_sw49410(panel, LGE_MP_NOR);
	else if (panel->lge.hl_mode)
		mplus_mode_send_sw49410(panel, LGE_MP_HBM);
	else if (panel->lge.mp_max != LGE_MP_OFF)
		mplus_mode_send_sw49410(panel, LGE_MP_FHB);
	else if (panel->lge.adv_mp_mode != LGE_MP_OFF)
		mplus_mode_send_sw49410(panel, panel->lge.adv_mp_mode);
	else
		mplus_mode_send_sw49410(panel, panel->lge.mp_mode);
}

void lge_set_screen_mode_sw49410(struct dsi_panel *panel, bool send_cmd)
{
	switch (panel->lge.screen_mode) {
	case LGE_COLOR_ECO:
	case LGE_COLOR_GAM:
	...
	case LGE_COLOR_CIN:
	case LGE_COLOR_SPO:
		panel->lge.adv_mp_mode = LGE_MP_GAL;
		break;
	case LGE_COLOR_MAN:
		if(panel->lge.color_filter)
	...
	mplus_mode_set_sub_sw49410(panel);
}

static void lge_set_rgb_tune_sw49410(struct dsi_panel *panel, bool send_cmd)
{
        mplus_mode_set_sub_sw49410(panel);
}

static void lge_set_screen_tune_sw49410(struct dsi_panel *panel)
{
        lge_set_screen_mode_sw49410(panel, true);
}

/* Hidden Menu Mplus Set */
enum lge_mplus_mode mplus_hd_get_sw49410(struct dsi_panel *panel)
{
	return panel->lge.mplus_hd;
}

void mplus_hd_set_sw49410(struct dsi_panel *panel, int mode)
{
...
int hl_mode_get_sw49410(struct dsi_panel *panel)
{
	return panel->lge.hl_mode;
}

void hl_mode_set_sw49410(struct dsi_panel *panel, int mode)
{
...
enum lge_mplus_mode mplus_max_get_sw49410(struct dsi_panel *panel)
{
	return panel->lge.mp_max;
}

void mplus_max_set_sw49410(struct dsi_panel *panel, int mode)
{
...
/* Advanced Mplus Set */
enum lge_mplus_mode mplus_mode_get_sw49410(struct dsi_panel *panel)
{
	return panel->lge.adv_mp_mode;
}

void mplus_mode_set_sw49410(struct dsi_panel *panel, int mode)
{
...
/* Normal Mplus Set */
enum lge_mplus_mode image_enhance_get_sw49410(struct dsi_panel *panel)
{
	return panel->lge.mp_mode;
}

void image_enhance_set_sw49410(struct dsi_panel *panel, int mode)
{
...
enum lge_blmap_type mp_blmap_sw49410_sub(struct dsi_panel *panel, enum lge_mplus_mode req_mode)
{
...
enum lge_blmap_type mp_blmap_sw49410(struct dsi_panel *panel)
{
	enum lge_blmap_type bl_type = LGE_BLMAP_DEFAULT;

	if (panel->lge.mplus_hd != LGE_MP_OFF)
		bl_type = mp_blmap_sw49410_sub(panel, panel->lge.mplus_hd);
	else if (panel->lge.mp_max != LGE_MP_OFF)
		bl_type = mp_blmap_sw49410_sub(panel, LGE_MP_FHB);
	else if (panel->lge.hdr_mode)
		bl_type = LGE_BLMAP_DEFAULT;
	else if(panel->lge.adv_mp_mode != LGE_MP_OFF)
		bl_type = mp_blmap_sw49410_sub(panel, panel->lge.adv_mp_mode);
	else
		bl_type = mp_blmap_sw49410_sub(panel, panel->lge.mp_mode);
	return bl_type;
}

int hdr_mode_set_sw49410(struct dsi_panel *panel, int input) {
...
void sharpness_set_sw49410(struct dsi_panel *panel, int mode)
{
...
struct lge_ddic_ops sw49410_ops = {
	...
	.lge_set_rgb_tune = lge_set_rgb_tune_sw49410,
	.lge_set_screen_tune  = lge_set_screen_tune_sw49410,
	.hdr_mode_set = hdr_mode_set_sw49410,
	.sharpness_set = sharpness_set_sw49410,
	.mplus_hd_get = mplus_hd_get_sw49410,
	.mplus_hd_set = mplus_hd_set_sw49410,
	.hl_mode_get = hl_mode_get_sw49410,
	.hl_mode_set = hl_mode_set_sw49410,
	.mplus_max_get = mplus_max_get_sw49410,
	.mplus_max_set = mplus_max_set_sw49410,
	.mplus_mode_get = mplus_mode_get_sw49410,
	.mplus_mode_set = mplus_mode_set_sw49410,
	.image_enhance_get = image_enhance_get_sw49410,
	.image_enhance_set = image_enhance_set_sw49410,
	.mp_blmap = mp_blmap_sw49410,
	.mp_blmap_sub = mp_blmap_sw49410_sub,
};
