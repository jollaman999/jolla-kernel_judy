...
static bool idtp9222_set_voltage(struct idtp9222_struct* idtp9222, int mV) {
	u8 voltage = -1;
	switch (mV) {
...
static bool idtp9222_set_overheat(struct idtp9222_struct* idtp9222) {
	...
	if (idtp9222->status_temperature >= idtp9222->configure_overheat) {
		...
		pr_idt(IDT_MONITOR, "The device is overheat, Send EPT_BY_OVERTEMP after 500ms\n");
		msleep(500);
		...
		if (idtp9222_reg_write(idtp9222->wlc_client, REG_ADDR_EPT, EPT_BY_OVERTEMP) &&
			...
			pr_idt(IDT_MONITOR, "Success to turn off TX\n");
...
static bool psy_set_onpad(struct idtp9222_struct* idtp9222, bool onpad) {
	if (onpad) {
		...
		*/
		 ...
		 * Skip setting capacity for WPC Test
	...
	if (idtp9222->status_onpad != onpad) {
		...
		// Check to overheat condition
		idtp9222_set_overheat(idtp9222);
		// Set Voltage Max
		idtp9222_set_voltage(idtp9222, idtp9222->status_voltage);
...
static int psy_property_set(struct power_supply* psy,
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		idtp9222_set_voltage(idtp9222, val->intval);
		break;
	default:
		break;
	}
